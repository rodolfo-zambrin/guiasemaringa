...
import { useMetaAds } from '@/hooks/useMetaData'
...
export default function MetaAnunciosPage() {
  const { dateRange } = useDashboardStore()
  const { data, isLoading } = useMetaAds(dateRange)
  ...
  const rows = useMemo(() => {
    ...
    for (const row of data ?? []) {
      ...
      const existing = map.get(key) ?? {
        ...
        status: row.status ?? '',
        spend: 0, impressions: 0, clicks: 0, ctr: 0, cpm: 0, cpc: 0, leads: 0, hook_rate: 0, hold_rate: 0,
      }
  ...
  const columns: ColumnDef<AdRow, unknown>[] = [
    {
      accessorKey: 'ad_name',
      header: 'Anúncio',
      cell: (info) => (
        <div>
          <p className="font-medium text-[#F1F5F9] text-sm leading-snug max-w-[220px] truncate">{info.getValue() as string}</p>
          <p className="text-xs text-[#64748B] truncate max-w-[220px]">{(info.row.original as AdRow).adset_name}</p>
        </div>
      ),
    },
    {
    ...
    { accessorKey: 'hook_rate', header: 'Hook Rate', cell: (info) => <span>{fmtPct(info.getValue() as number)}</span> },
    { accessorKey: 'hold_rate', header: 'Hold Rate', cell: (info) => <span>{fmtPct(info.getValue() as number)}</span> },
  ]

  return (
    <div className="flex flex-col flex-1">
      <Header title="Anúncios — Meta Ads" showPlatformSelector={false} />
        ...
        <div className="bg-[#1E293B] border border-[#334155] rounded-xl p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-sm font-semibold text-[#F1F5F9]">
              {rows.length} anúncios · {dateRange.from} a {dateRange.to}
            </h3>
          </div>
